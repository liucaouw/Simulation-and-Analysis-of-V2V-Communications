%CSMA simulation(fully connected vehicular network)
lambda=10;
data_rate=6*10^6;
Packet_size=250*8;
repeat_times=1;
T=round(10000 * 10^-3/slot_size);
slot_size=16*10^-6;
...
while N<=Nm %vehicle density%
    ...
    every_stan_delay=zeros(1,repeat_times);
   ...
   ave_stand_delay(N/10)=ave_stand_delay(N/10)/repeat_times;
  ...
  for i=1:repeat_times
      ...
      mmmm(N/10)=mmmm(N/10)+(ave_stand_delay(N/10)-every_stan_delay(i))^2/repeat_times;
  end
