%Contention Intensity control with rate control
function A1=Simu_CICRC()
    global lambda;
    global data_rate;
    global Packet_size;
    global repeat_times;
    ...
    Nm=300;%maximum vehicle density
    ...
    lambda_changes=zeros(1,Nm/10);
    
while N<=Nm %vehicle density%
    k=1;
    every_delivery_ratio=zeros(1,repeat_times);%used to calculate standard deviation%
    every_ave_delay=zeros(1,repeat_times);
    lambda_change=min(lambda,rg/((N+15)*(Packet_size/data_rate+2*slot_size)));
   ...
   while k<=repeat_times  % repeating times for one density%
    ...
    r = round(((randi([0,round((1/lambda_change)*10^6)],1,N))*10^-6/slot_size)+DIFS);% generate N counters randomly and put them into an array %
    backoff_flag=zeros(1,N);%set the backoff flag for each vehicle%
    delay=zeros(1,N);%initial vehicle delay%
    delay_total=zeros(1,N);
    to_delay=0;
    q=1/lambda_change*ones(1,N);% generate next N counters randomly and put them into an array %
    aaaaa=0;
    bb=zeros(1,N);
    ...
    every_ave_delay(k)=to_delay*slot_size*1000/(good_transfer+bad_transfer);
    k=k+1; 
    end
   ...
   d_ratio(N/10)=d_ratio(N/10)/repeat_times;
    for i=1:repeat_times
      ...
      delay_standard_deviation(N/10)=delay_standard_deviation(N/10)+(every_ave_delay(i)-ave_delay(N/10))^2/repeat_times;
    end
    standard_deviation(N/10)=sqrt(standard_deviation(N/10));
    delay_standard_deviation(N/10)=sqrt(delay_standard_deviation(N/10));
    reception_time(N/10)=reception_time(N/10)+1000/(lambda_change*d_ratio(N/10))+ave_delay(N/10);
    lambda_changes(N/10)=lambda_changes(N/10)+lambda_change;
    N=N+10;
end
