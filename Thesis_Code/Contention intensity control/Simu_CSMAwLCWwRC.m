%CSMA with large contention window with rate control (fully connected vehicular network)
function A1=Simu_CSMAwLCWwRC()
    global lambda;
    global data_rate;
    global Packet_size;
...
while N<=Nm %vehicle density%
    ...
    every_stan_delay=zeros(1,repeat_times2);
             ...
             if (M == r(i))                
                 if(aaaa>0)%only jumping
                    if(l<aaaa)
                       ...
                       q(i)=1/lambda_change;%generate the next counter of vehicle i  
   ...
   ave_stand_delay(N/10)=ave_stand_delay(N/10)/repeat_times2;
  ...
  for i=1:repeat_times2
      ...
      mmmm(N/10)=mmmm(N/10)+(ave_stand_delay(N/10)-every_stan_delay(i))^2/repeat_times2;
  end
