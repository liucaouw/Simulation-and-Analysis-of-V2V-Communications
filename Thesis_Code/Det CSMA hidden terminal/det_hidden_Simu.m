...
while N<=Nm %vehicle density%
   ...
   while k<=repeat_times  % repeating times for one density%
    ...
    to_delay=0;
    m=1;
    q=ones(1,N)/lambda;% generate next N counters randomly and put them into an array %
    detect=zeros(1,N);%detect the delay in the first DIFS% 
    
    total_time_1 = 0;
    collision_flag_1 = 0;
    r_1 = round(((randi([0,(1/lambda)*10^6],1,N))*10^-6/slot_size)+DIFS);% generate N counters randomly and put them into an array %
    ...
    m_1=1;
    q_1=ones(1,N)/lambda;% generate next N counters randomly and put them into an array %
    ...
    total_time_2 = 0;
    collision_flag_2 = 0;
    r_2 = round(((randi([0,(1/lambda)*10^6],1,N))*10^-6/slot_size)+DIFS);% generate N counters randomly and put them into an array %
             ...
             if (M_1 == r_1(i))                
                 if(aaaa>0)%only jumping
                    if(l<aaaa)
                       ...
                       q_1(i)=1/lambda;%generate the next counter of vehicle i  
             ...
             if (M_2 == r_2(i))                
                 if(aaaa>0)%only jumping
                    if(l<aaaa)
                       ...
                       q_2(i)=1/lambda;%generate the next counter of vehicle i  
